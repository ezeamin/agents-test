...
## Chatterbox TTS — pipelining strategy
...
**`ChatterboxServerTTSPipelined`** (`TTS_SERVICE_PROVIDER=CHATTERBOX_SERVER_PIPELINED`)
fires one `/tts` request per sentence immediately as a background `asyncio.Task`,
without waiting for the previous sentence to finish generating. Each sentence gets
its own `asyncio.Queue` (audio context). A background `_audio_context_handler` task
drains these queues in arrival order so audio always plays sentence-by-sentence —
sentence N+1 is already being generated by the server while sentence N is playing.
This eliminates the inter-sentence gap caused by pipecat's base class awaiting the
full `run_tts` generator before starting the next sentence.

**`ChatterboxServerTTS`** (`TTS_SERVICE_PROVIDER=CHATTERBOX_SERVER`, default) is
the stable sequential fallback — one request at a time, no pipelining.

> **WAV header assumption:** `ChatterboxServerTTSPipelined` skips the first 44 bytes
> of each HTTP response to strip the WAV header and get raw PCM. This works for
> standard PCM WAV files (which is what Chatterbox generates), but would break if
> the server ever uses extended WAV chunks with a variable-length header. Fix: parse
> the `data` chunk offset instead of hardcoding 44 bytes.

**Deprecated:** `ChatterboxServerTTSSentenceSplit` — the earlier per-client
sentence-split approach added more latency than it saved because pipecat still
awaited each `/tts` request serially before sending the next. The pipelined class
solves the underlying problem at the asyncio level.

When the upstream streaming PR lands, `ChatterboxServerTTSPipelined` can be
replaced with the base `ChatterboxServerTTS` (once it yields audio as it generates
